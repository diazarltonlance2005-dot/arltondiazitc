...
{ 
   ...
   sessionStorage.setItem("answer","x");
   sessionStorage.setItem("confirmation","no");
   sessionStorage.setItem("itemno",-1);
...
// compute amount;
 function computeamount()
  ...
  function generateResponse()
  {
        ...
        console.log(question+" "+answer+" "+confirmation+" "+itemno);
        if (question=="Clear the cart?" && answer=="confirm" && confirmation=="yes"){ 
               
                initcart();
               showcart();
         }  else if (question=="Delete the item"   && answer=="confirm" )
         {
                cart.splice(itemno,1);
                showcart();
  ...
  document.getElementById("btnaddtocart").addEventListener("click",
   ...
   ()=>{
    ...
    btnConfirm.addEventListener("click", () => {
        sessionStorage.setItem("answer","confirm")
   ...
   btnCancel.addEventListener("click", () => {
       sessionStorage.setItem("answer","cancel")
    ...
    dlgConfirmCancel.addEventListener("close",(e)=>
    {
        ...
        sessionStorage.setItem("answer", rst);

        if (rst=="confirm") {
            if (lblmsg=="Clear the cart?" ){ 
               sessionStorage.setItem("question",lblmsg);
               dlgAreYouSure.showModal();

            }  else if (lblmsg.substring(0,15)=="Delete the item"){
                var x=document.getElementById("delitem").value*1 - 1;
                sessionStorage.setItem("question","Delete the item");
                sessionStorage.setItem("itemno",x);     
                generateResponse();
            } 
        }
    ...
    dlgAreYouSure.addEventListener("close",(e)=>
    {
        var rst= e.target.returnValue;
     //   var lblmsg = document.getElementById("dlgmsg").innerHTML;
        sessionStorage.setItem("confirmation",rst);
         generateResponse();
